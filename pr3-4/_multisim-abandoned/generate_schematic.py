#!/usr/bin/env python3
"""
Генератор схемы KiCad для ПР №3-4 «Подключение внешней памяти к МК-51»
(ТЗ: «Практическое занятие 3-4 _Multisim.pdf», аналог рис.25 методички):
...
Формат KiCad 10. Студент: ЭФБО-04-24, Валекжанин В.С., вариант 4
(Задание 1 = табл. 8 стр. 4; Задание 2 = табл. 9 стр. 10).
...
sys.path.insert(0, os.path.expanduser("~/KiCAD-MCP-Server/python"))

GRID = 1.27
ROOT = Path(__file__).resolve().parent
KICAD = ROOT / "kicad"
SCH = KICAD / "pr3-4.kicad_sch"
PRO = KICAD / "pr3-4.kicad_pro"
...
_spec = importlib.util.spec_from_file_location(
    ...
    os.path.expanduser("~/KiCAD-MCP-Server/python/commands/dynamic_symbol_loader.py"),
)
...
def main():
    ...
    ap.add_argument("--variant1", type=int, default=4)
    ap.add_argument("--variant2", type=int, default=10)
    a = ap.parse_args()
...
\t(paper "A3")
\t(title_block
\t\t(title "ПР №3-4. Подключение внешней памяти к МК-51 (аналог рис.25)")
\t\t(date "{date.today().isoformat()}")
\t\t(rev "1.0")
\t\t(company "РТУ МИРЭА, ЭФБО-04-24, Валекжанин В.С., вариант 4")
\t)
\t(lib_symbols
\t)
\t(sheet_instances
...
''',
    ...
    loader = DynamicSymbolLoader(project_path=KICAD)
    b = B(SCH)
    ...
    place(
        ...
        "74LS373",
        "U3",
        "74LS373 (защелка A0-A7)",
        LAT_X,
        LAT_Y,
        fp="Package_DIP:DIP-20_W7.62mm",
    )
    place(
        "Memory_RAM",
        "HY6264AxP",
        "U2",
        f"HY6264 (ОЗУ {v1['N_kb']}Кx8, Вар.{v1['variant']})",
        RAM_X,
        RAM_Y,
        fp="Package_DIP:DIP-28_W15.24mm",
    )
    place(
        "Memory_EPROM",
        "27C64",
        "U4",
        f"27C64 (ПЗУ {v2['N_kb']}Кx8, Вар.{v2['variant']})",
        ROM_X,
        ROM_Y,
        fp="Package_DIP:DIP-28_W15.24mm",
    )
    ...
    b.text(
        "R1 330 Ом: P1.0 -> D1 (ERROR). Горит при несовпадении write/read.",
        130,
        225,
        1.5,
    )
    ...
    stub_label(
        Y1_X, Y1_Y, *P_XT["1"][0][:2] if "1" in P_XT else (P_XT[""][0][:2]), "XTAL1"
    )
    _xt_keys = list(P_XT.keys())
    _xt_all = [t for _lst in P_XT.values() for t in _lst]
    _xt2 = (
        [t for t in _xt_all if str(t[2]) == "2"][0]
        if any(str(t[2]) == "2" for t in _xt_all)
        else _xt_all[1]
    )
    ...
    b.text(
        "+5V/GND: питание U1-U5, pull-up CS2/VPP/PGM/EA/E3 через метку +5V.",
        80,
        95,
        1.5,
    )
    ...
    b.text(
        f"Карта памяти (Вар.1={v1['variant']}, Вар.2={v2['variant']}): ОЗУ {v1['N_kb']}К в 0000h..; тест 1Кбайт с {v1['ZZZ']} паттерн {v1['XX']}; ПЗУ {v2['N_kb']}К; CRC {v2['K_bytes']} байт с {v2['ZZZ']}.",
        40,
        40,
        1.7,
    )
    b.text(
        "Дешифратор U5 74LS138: RAM_CS=O0 (0000h-1FFFh = !A15&!A14&!A13); ROM_CS=O4 (окно 8000h-9FFFh, полное ПЗУ — ИЛИ O4-O7). Для 2К A11-A12 к GND.",
        40,
        46,
        1.5,
    )
    b.text(
        "ALE: 2 импульса за машинный цикл (1 мкс при 12 МГц). Осциллограф XSC1: 1-Ch.A -> ALE, 2-Ch.B -> GND; 1 мкс/дел, 5 В/дел.",
        40,
        52,
        1.5,
    )
    ...
    b.text(
        "P0: AD0-AD7 мультиплекс (адрес при ALE=1, данные при ALE=0). U3 фиксирует A0-A7 по спаду ALE. P2: A8-A15. /RD P3.7, /WR P3.6, /PSEN к ПЗУ.",
        40,
        58,
        1.5,
    )
    b.text(
        "Соответствие Multisim рис.25: Bus1=AD0-AD7, Bus2=A0-A7 (Merge), U3=74LS373N (~4037BP), U2=RAM 6116/6264.",
        40,
        64,
        1.5,
    )
