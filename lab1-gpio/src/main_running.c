...
int main(void) {
    ...
    //    ИСПРАВЛЕНО: в методичке цикл был i < 3 (настраивал только PA0–PA2);
    //    здесь i < 4 — настраиваются все четыре вывода PA0–PA3.
    for (int i = 0; i < 4; i++) {
    ...
    // 3. Основной цикл: бегущий огонь PA0 -> PA3
    uint8_t pattern = 0x01U; // 0000 0001 — старт с PA0

    while (1) {
        // Маска 0xFFF0 сохраняет состояние PA4–PA15 и обнуляет PA0–PA3,
        // затем | pattern вставляет новый шаблон.
